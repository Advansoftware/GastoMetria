...
// Simple HTTP server implementation for React Native
class SimpleHTTPServer {
  ...
  async listen(host: string, callback: () => void) {
    // For React Native, we'll simulate a server using a service worker approach
    // This is a simplified implementation that stores the handlers
    this.isRunning = true;
    callback();
    return this;
  }
  ...
  async handleRequest(method: string, path: string, query?: any): Promise<any> {
...
class LocalServerService {
  ...
  private setupNetworkListener() {
    // Listen for network changes (expo-network doesn't have addEventListener)
    // We'll check network status manually when needed
  }
  ...
  private setupRoutes() {
    ...
    // Items endpoint
    this.server.get('/api/items', async () => {
      try {
    ...
    // Products endpoint
    this.server.get('/api/products', async () => {
      try {
        const storedData = await AsyncStorage.getItem('gastometria_items');
        const items = storedData ? JSON.parse(storedData) : [];
    ...
    this.server.get('/api/establishments', async () => {
      try {
        const storedData = await AsyncStorage.getItem('gastometria_items');
        const items = storedData ? JSON.parse(storedData) : [];
  ...
  async handleAPIRequest(method: string, path: string, query?: any): Promise<any> {
    ...
    return await this.server.handleRequest(method, path, query);
  }
  ...
  async start(): Promise<boolean> {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        try {
          this.server = new SimpleHTTPServer(this.config.port);
          this.setupRoutes();
          
          this.server.listen('0.0.0.0', () => {
